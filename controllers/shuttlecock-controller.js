...
const shuttlecockController = {
  getAll: async (req, res, next) => {
    ...
    try {
      ...
      const [shuttlecocks] = await connection.query(`SELECT s.*, b.name as branch FROM shuttlecocks AS s JOIN branches AS b ON b.id = s.branchId LIMIT ${limit} OFFSET ${offset}`)
  ...
  getShuttlecock: async (req, res, next) => {
    ...
    try {
      connection = await global.pool.getConnection()
      const [shuttlecock] = await connection.query('SELECT s.*, b.name as branch FROM shuttlecocks AS s JOIN branches AS b ON b.id = s.branchId WHERE s.id = ?', [shuttlecockId])
